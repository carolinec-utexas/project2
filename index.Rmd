...
#HDI: HDI>0.5=MDC (1), HDI<0.5=LDC (0)
...
#final data averages across the 5 year period
...
I used a loop to decide how many clusters to pick based on the largest average silhouette width. The silhouette widths were plotted for when k was equal to 2 through 10 using ggplot, and k=3 (3 clusters) had the highest average silhouette width. At k=3, the clusters are both the most cohesive and separated. The average silhouette width is 0.769, which indicates that there is a "strong structure," so the cluster solution is good. 

The pairwise plots are pretty similar. However, the red cluster is very separated from the green and blue clusters in the HDI plot. This indicates that most of the countries over the 5 year period either remained an LDC or remained an MDC. Very few countries changed their status (either from LDC to MDC or vice versa). The greatest negative correlation (-0.577) is between the percent of land covered in forest and the percent of land used for agriculture. Additionally, there is a negative correlation (-0.507) between HDI and the number of malaria cases (per 100,000 people). This could indicate a couple patterns. Countries that have low HDI scores might happen to be located where malaria is most common (around the equator), and/or countries that have low HDI scores might not have their malaria cases as well under control as countries with high HDI scores. The variables with the strongest positive correlation are "forest" and "malaria;" however, the correlation is not very strong overall (0.153). The variables that show the least correlation are "agriculture" and "livestock" (-0.039), which is interesting since these variables are both related to agricultural production. Based on the cluster scatterplots, the countries in the red cluster tend to have higher HDI scores and very low rates of malaria incidence, the countries in the green cluster tend to have low HDI scores and moderates rates of malaria incidence, and the countries in the blue cluster have low HDI scores and the highest rates of malaria incidence. There do not appear to be other noteworthy trends. However, I think it is interesting that the countries with high HDI scores had slightly less land covered in forest (red cluster), and countries in the blue cluster (low HDI scores) had slightly more land used for agriculture. I think that one reason why the countries with high HDI scores might have less forest cover is they could be more urbanized than countries with low HDI scores. Additionally, one potential reason why countries with low HDI scores have more land used for agriculture is their economies might be agriculture-based, whereas countries with higher HDI scores are more likely to have industry and service-based economies. 
...
PC1 represents Agriculture versus Forest Cover and HDI. Countries that have a higher percent of land for agriculture have lower HDI scores and have a lower percent of land covered in forest. PC2 represents Malaria Incidence versus HDI. Countries with a high incidence of malaria tend to have lower HDI scores, and countries with a low incidence of malaria tend to have higher HDI scores. PC3 represents Livestock versus HDI. Countries with higher livestock index scores tend to have lower HDI scores, and countries with lower livestock index scores tend to have higher HDI scores. 82.71% of the total variance in the dataset is explained by PC1, PC2, and PC3. I think it makes sense that countries with lower HDI scores tend to have relatively more land devoted to agriculture (PC1) and also have higher livestock index scores (PC3) since these countries are more likely to have agriculture-based economies. Additionally, since countries with higher HDI scores probably have more access to medical resources, it is makes sense that these countries also have lower rates of malaria incidence (PC2).
...
#confusion matrix
table(truth= factor(LC_data$avg_HDI==1), prediction=factor(prob_project[]>.5))
...
library(caret)
knn_fit <- knn3(avg_HDI == "1" ~ malaria+ livestock+ agriculture+ forest, data = LC_data, k=10)

prob_knn <- predict(knn_fit, LC_data)
class_diag(prob_knn[, 2], LC_data$avg_HDI, positive = "1")

## confusion matrix
table(truth=factor(LC_data$avg_HDI==1), prediction=factor(prob_knn[,2]>.5))
...
Discussion: When I used k-nearest neighbors to predict the binary variable (HDI) from all of my numeric variables (malaria, livestock, forest, and agriculture) the AUC was 0.9041, which indicates that the model is doing pretty well. When I performed a k-fold CV, and the model did not predict new observations as well as indicated by the decrease in the CV AUC (AUC=0.8484). Since the AUC dropped in the CV, this indicates that there is likely overfitting. Compared to the linear model, the nonparametric model is doing better in terms of its cross-validation performance. Since the nonparametric model's CV AUC (0.8484) is higher than the linear model's CV AUC (0.7848), this indicates that the CV performance for the nonparametric model is better. 
...
data<-reg_data[sample(nrow(reg_data)),] #randomly order rows
folds<-cut(seq(1:nrow(reg_data)),breaks=k,labels=F) #create folds
...
Discussion: The MSE for the overall dataset is 270.4508, which is very high. The average MSE across the k testing folds was 347.745, which is even higher and indicates that there is overfitting. 
...
Discussion: In the python code chunk, I created an object called "pathogen" that prints "knowlesi," and in the R code chunk, I created another object called "pathogen" that prints "Plasmodium." Reticulate was used to share the objects between R and python. In the python code chunk, "r.pathogen" shared the R object with python, and in the R code chunk, "py$pathogen" was used to share the python object with R. Plasmodium knowlesi is a species of malaria parasite that is unique because it is zoonotic (i.e. it can be transmitted between animals and humans).

### Concluding Remarks

In this project, it was interesting to compare the CV AUCs of the different types of models and to determine which kind of model fit the data best. In future studies, it could be interesting to compare the livestock, forest, and agriculture data to the incidence of zoonotic infectious diseases, such as Nipah and Sars-CoV-2 because these pathogens have well-documented relationships with agricultural expansion and deforestation. Additionally, many noteworthy zoonoses (Ebola, SARS, SARS-CoV-2, Hendra, Nipah, MERS, etc.) likely use bats as reservoir hosts. Therefore, it would be interesting to conduct the above analyses that included data on the a) distribution of these bat species and b) percent of land that serves as suitable bat habitat. I am curious to know how these variables change over time. Also, it could be interesting to see how well this data can be used to predict the incidence of zoonoses. These models might have useful applications in predicting where future outbreaks of zoonoses are most likely to occur.
